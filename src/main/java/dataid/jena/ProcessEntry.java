...
public class ProcessEntry {
	...
	public boolean saveNewEntry(DynamicLODCloudEntryModel entry) {
		...
		try {
			...
			r.addProperty(DynamicLODCloudEntry.byteSize,
					String.valueOf(entry.getByteSize()));
			r.addProperty(DynamicLODCloudEntry.dataIDFilePath,
					entry.getDataIDFilePath());
			r.addProperty(DynamicLODCloudEntry.objectPath,
					entry.getObjectPath());
			r.addProperty(DynamicLODCloudEntry.subjectFilterPath,
					entry.getSubjectFilterPath());
			r.addProperty(DynamicLODCloudEntry.subsetURI, entry.getSubsetURI());
			r.addProperty(DynamicLODCloudEntry.datasetURI,
					entry.getDatasetURI());
			r.addProperty(DynamicLODCloudEntry.timeToCreateFilter,
					String.valueOf(entry.getTimeToCreateFilter()));
			r.addProperty(DynamicLODCloudEntry.numberOfTriplesLoadedIntoFilter,
					String.valueOf(entry.getNumberOfTriplesLoadedIntoFilter()));
			r.addProperty(DynamicLODCloudEntry.numberOfObjectTriples,
					String.valueOf(entry.getNumberOfObjectTriples()));
	...
	private void updateDataID(String dataID, String dataIDUpdated, int links,
			String subsetURL, String linkURL, String datasetURI) {
		...
		String linksetSubject = subsetURL + randomGenerator.nextInt();
	...
	public void compareAllDistributions() {
		...
		try {
			...
			while (objectIterator.hasNext()) {
				...
				// get filter path
				StmtIterator filterPathIterator = fsModel.listStatements(null,
						DynamicLODCloudEntry.subjectFilterPath, (RDFNode) null);

				while (filterPathIterator.hasNext()) {
					Statement filterPath = filterPathIterator.next();

					// do not compare distribution with itself
					if (stmtObject.getSubject().toString()
							.equals(filterPath.getSubject().toString())) {
						System.out
								.println("No needs to compare the distribution with itself.");
						...
						// saving time to compare object list with filter
						Timer t = new Timer();
						t.startTimer();
						int numbersOfTriples = f.searchFileOnFilter(g,
								stmtObject.getObject().toString());
						addComparingTime(stmtObject.getSubject().toString(),
								filterPath.getSubject().toString(),
								t.stopTimer());
						...
						if (numbersOfTriples > 0) {
							...
							// get dataID path
							StmtIterator s = fsModel.listStatements(
									stmtObject.getSubject(),
									DynamicLODCloudEntry.dataIDFilePath,
									(RDFNode) null);
							if (s.hasNext())
								dataIDPath = s.next().getObject().toString();

							// get dataIDUpdated path
							s = fsModel.listStatements(stmtObject.getSubject(),
									DynamicLODCloudEntry.subsetURI,
									(RDFNode) null);
							if (s.hasNext())
								subsetURL = s.next().getObject().toString();

							// get target dataset link
							s = fsModel.listStatements(filterPath.getSubject(),
									DynamicLODCloudEntry.datasetURI,
									(RDFNode) null);
							if (s.hasNext())
							...
							updateDataID(dataIDPath, dataIDUpdatedPath,
									numbersOfTriples, subsetURL, linkURL,
									datasetURL);
	...
	private void addComparingTime(String objectURI, String filterURI,
			String time) {
		try {
			Model fsModel2 = ModelFactory.createDefaultModel();

			File f = new File(DataIDGeneralProperties.FS_MODEL);
			if (f.exists())
				fsModel2.read(DataIDGeneralProperties.FS_MODEL, "N-TRIPLES");
			Random randomGenerator = new Random();
			int rand = randomGenerator.nextInt();

			Resource newR = fsModel2.createResource(objectURI + "&comparingURI=" + filterURI);
			newR.addProperty(ComparingTime.ObjectURI, fsModel2.getResource(objectURI));
			newR.addProperty(ComparingTime.FilterURI, fsModel2.getResource(filterURI));
			newR.addProperty(ComparingTime.Time, time);

			Resource r = fsModel2.getResource(objectURI);
			r.addProperty(DynamicLODCloudEntry.comparision, newR);
			

				fsModel2.write(new FileOutputStream(
						DataIDGeneralProperties.FS_MODEL), "N-TRIPLES");
				fsModel2.close();
			} catch (FileNotFoundException e) {
				// TODO Auto-generated catch block
